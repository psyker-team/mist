...
with gr.Blocks() as demo:
    with gr.Row():
        with gr.Column():
            ...
            eps = gr.Slider(0, 32, step=4, value=16, label='Strength', info="Larger strength results in stronger defense at the cost of more visible noise.")
            steps = gr.Slider(0, 1000, step=1, value=100, label='Steps', info="Larger steps results in stronger defense at the cost of more running time.")
            input_size = gr.Slider(256, 768, step=256, value=512, label='Input size', info="Size of the input images.")

            mode = gr.Radio(["Textural", "Semantic", "Fused"], value="Fused", label="Mode", info="See documentation for more information about the mode") 
 
            with gr.Accordion("Parameters of fused mode",open=False):
              rate = gr.Slider(0, 5, step=1, value=1, label='Fusion weight', info="Higher fusion weight leads to more emphasis on \"Semantic\" ")

            block_mode = gr.CheckboxGroup(["Low VRAM usage mode"], info="Use this mode if the VRAM of your device is not enough. Check the documentation for more information.", label='VRAM mode')      
...
demo.queue().launch(share=True, server_name="0.0.0.0", server_port=8000)
